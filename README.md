...
- **BOT_TOKEN**: The private token for your bot. Make sure to keep it secret. This is required and must be set.
- **NOW_PLAYING**: The text that the bot will display as its now playing status. You can set it to anything *falsy* (`""`, `None`, `False`, ...) to disable it. Defaults to `COMMAND_PREFIX + "commands"`.
- **PORT**: The port number for the keep-alive web server. Defaults to 8080.
...
The bot also starts a simple web server on port 8080 (configurable via PORT environment variable) that you can ping to check if the bot is running. Visit `http://your-server:8080/ping` for a health check response.
